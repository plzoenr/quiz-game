...
const App: React.FC = () => {
    ...
    const handleAnswerSelection = (questionIndex: string, selectedOption: string, actualValue: string, actualIndex: number): void => {
        ...
        let element = document.querySelector<HTMLInputElement>(`input[name=${questionIndex}]`);

        if (element) {
            let parentElement = element.parentNode as HTMLInputElement
                ...
                {loading ? (
                    ...
                    <>
                        <form className="form" onSubmit={handleSubmit}>
                            ...
                            {quiz.length > 0 ? (<button type="submit" className="btn btn-primary mt-2 w-100">Submit</button>)
                                : (<Link to={{pathname: "/"}} onClick={refreshPage}>If quiz not show click this</Link>)
                            }
                        </form>
                    </>
                )}
